...
|---|---|---|---|
| Header + Hero | `Hero`, `Header`, `Specimen` | `35:228` (Hero 1440×820), nav `35:229`, specimen `35:283`/`35:284` | canonical |
...
Exact Figma exports in `public/assets/svg` (icons, hero specimen slabs, discipline illustrations,
network lines, wordmark) and `public/assets/img` (logos, avatars, stamps, card headers, accreditation
...
10. **Hero specimen in 3D** — the A2 Three.js comparison is isolated to the internal
    `/design-lab/hero/a2-three` route. The homepage default uses the approved H portrait loop and
    the SVG specimen remains the fallback for reduced motion, no WebGL and phones. The A2 lab's
    fixed orthographic camera has no pointer response, orbit, drift or decorative lighting.
...
14. **Console** — `THREE.Clock` deprecation warning comes from React Three Fiber 9.7 with three
    0.185 (library internal); the only failed request is the missing licensed Coolvetica file (see 1).
...
20. **Correction pass (2026-09-02, evening)** — Hero: the Three.js scene is no longer live in the page;
    it renders an 8 s seamless loop offline (`scripts/hero-loop-render.mjs` → `public/assets/media/
    hero-loop.{webm,mp4}` + poster) that plays muted/inline/looping in the exact 401×542 Figma slot,
    with the SVG specimen as the server-rendered and reduced-motion state. Footer circle is clipped to
    end 56px under the CTA buttons with a soft fade. Buttons: hover = clear colour step, a quick
...
21. **Hero A2 comparison lab (2026-09-03)** — internal, non-indexed routes are intentionally limited
    to `/design-lab/hero/a2-svg`, `/design-lab/hero/a2-three` and the unchanged Hero H film baseline
    at `/design-lab/hero/h`. Both A2 routes reuse the approved eight-slab Option A specimen geometry
    and show the same complete → single-axis separation → labelled inspection → reassembly loop. The
    SVG route uses one GSAP timeline; the Three.js route fixes its orthographic camera and removes
    pointer response, orbit, drift and decorative lighting so depth is the only comparison variable.
    Reduced motion resolves to the labelled exploded SVG state without a canvas or loop. One shared
    A2 model owns the five callouts and timing for both renderers; a separate logically ordered text
    description exposes the metaphor once while both visual renderers stay hidden from assistive
    technology. Mobile callouts remain 10px rather than shrinking to annotation-sized 8px text. This
    lab does not change the homepage default.
22. **Hero H production loop (2026-09-03)** — H is the homepage default. One original environmental
    portrait fills the hero behind the fixed navigation, headline and calls to action. Desktop and
    tablet widths crossfade through the four approved portrait images with a 4.5-second hold and a
    720ms transition. A single navy scrim preserves copy contrast without a smoky top/bottom feather,
    and every image keeps its calibrated crop. There are no teaser tiles, role labels, captions,
    portrait videos or moving copy. Reduced motion holds the opening portrait. Phone-sized layouts
    intentionally use an image-free navy lockup, while the legacy shutter loop remains available only
    at the internal `/design-lab/hero/h` comparison route.

23. **Site finishing pass and Hero H housekeeping (2026-09-03)** — Client profile imagery keeps its
